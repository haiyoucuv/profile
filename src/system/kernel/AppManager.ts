...
import { SystemContext, SystemEvent } from "./SystemContext.ts";
import { WindowManager } from "./WindowManager/WindowManager.ts";
...
export class AppManager extends Emittery<{ [key: symbol]: any }> {
    ...
    private _ipcCleanups: Map<string, Set<() => void>> = new Map();

    public ipcEmitter = new Emittery();
    ...
    async launchAppById(appId: string, ...args: any[]): Promise<VirtualApp | null> {
        ...
        this.ipcEmitter.emit('ipc-broadcast', { event: SystemEvent.APP_LOADING, data: appId, sourceAppId: 'kernel' });
        ...
        try {
            ...
            // 注入上下文 SystemContext
            const ipcWrappers = new Map<Function, any>();
            const ipcCleanups = new Set<() => void>();
            this._ipcCleanups.set(appId, ipcCleanups);

            const sys: SystemContext = {
                window: {
                    create: (options) => {
                        const win = WindowManager.ins.createWindow(options);
                        app.registerWindow(win);
                        return win;
                    },
                    close: (win: SystemWindow) => {
                ...
                ipc: {
                    send: (targetAppId: string, event: SystemEvent | string, data?: any) => {
                        AppManager.ins.ipcEmitter.emit(`ipc-message:${targetAppId}`, { event, data, sourceAppId: appId });
                    },
                    on: (event: SystemEvent | string, callback: (data: any, sourceAppId: string) => void) => {
                        const handler = ({ event: e, data, sourceAppId }: any) => {
                            if (e === event) callback(data, sourceAppId);
                        };
                        ipcWrappers.set(callback, handler);
                        AppManager.ins.ipcEmitter.on(`ipc-message:${appId}`, handler);
                        AppManager.ins.ipcEmitter.on(`ipc-broadcast`, handler);
                        const unsubscribe = () => {
                            AppManager.ins.ipcEmitter.off(`ipc-message:${appId}`, handler);
                            AppManager.ins.ipcEmitter.off(`ipc-broadcast`, handler);
                            ipcWrappers.delete(callback);
                            ipcCleanups.delete(unsubscribe);
                        };
                        ipcCleanups.add(unsubscribe);
                        return unsubscribe;
                    },
                    off: (event: SystemEvent | string, callback: (data: any, sourceAppId: string) => void) => {
                        const handler = ipcWrappers.get(callback);
                        if (handler) {
                            AppManager.ins.ipcEmitter.off(`ipc-message:${appId}`, handler);
                            AppManager.ins.ipcEmitter.off(`ipc-broadcast`, handler);
                            ipcWrappers.delete(callback);
                    ...
                    broadcast: (event: SystemEvent | string, data?: any) => {
                        AppManager.ins.ipcEmitter.emit('ipc-broadcast', { event, data, sourceAppId: appId });
                    }
                },
                exit: () => {
                ...
                app: {
                    launch: (id: string) => this.launchAppById(id),
                    list: () => AppRegistry.ins.getDockerAppInfos(),
                    isRunning: (id: string) => this.isAppRunningById(id),
                    getWindows: (id: string) => {
                        const app = this.getAppById(id);
                        return app ? (Array.from(app.windows.values()) as unknown as SystemWindow[]) : [];
                    }
                }
            };
            app.sys = sys;
            ...
            this.ipcEmitter.emit('ipc-broadcast', { event: SystemEvent.APP_CHANGE, data: appId, sourceAppId: 'kernel' });
            this.emit(AppManager.EventType.ON_APP_LOADED, { appId, app });
            this.ipcEmitter.emit('ipc-broadcast', { event: SystemEvent.APP_LOADED, data: appId, sourceAppId: 'kernel' });
    ...
    exitApp(AppClass: TAppConstructor<VirtualApp>) {
        ...
        const appId = (AppClass as any).id;
        const cleanups = this._ipcCleanups.get(appId);
        if (cleanups) {
            cleanups.forEach(unsub => unsub());
            this._ipcCleanups.delete(appId);
        }
