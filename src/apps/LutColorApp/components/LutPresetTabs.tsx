  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.tabHeader}>
        ...
        >
          基础预设
          <span className={styles.count}>({basicPresets.length})</span>
        </button>
        <button
        ...
        {isInitializing ? (
          ...
          <div className={styles.presetGrid}>
            {currentPresets.map(preset => (
              <button
                key={preset.id}
                className={`${styles.presetButton} ${isPresetActive(preset) ? styles.active : ''} ${isLoading ? styles.disabled : ''}`}
                onClick={() => handlePresetClick(preset)}
                disabled={isLoading}
                title={preset.name}
              >
                <span className={styles.presetName}>{preset.name}</span>
                {preset.type === 'advanced' && (
                  <span className={styles.presetType}>LUT</span>
                )}
              </button>
            ))}
